...
import 'sortable_list_item.dart';
import 'sortable_overlay.dart';

class SortableListState<T> extends State<SortableList<T>>
  ...
  bool get _dragsForwards => (_boundedDragDelta ?? 0) > 0;
  ...
  @override
  void didUpdateWidget(SortableList<T> oldWidget) {
  ...
  Widget _buildOverlay(BuildContext context) {
    return SortableOverlay(
      ...
      child: widget.feedbackItemBuilder(
        context,
        SortableItem(widget.items[_dragIndex!], _dragIndex, _hoverIndex),
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildSortableItem(context, itemIndex, index);
      },
    );
  }
  ...
  Widget _buildSortableItem(BuildContext context, int itemIndex, int dispIndex) {
    return SortableListItem(
      ...
      builder: (context, handle) => widget.itemBuilder(context,
          SortableItem(widget.items[itemIndex], itemIndex, dispIndex), handle),
  ...
  void _runDropAnim(Offset stopOffset) {
    ...
    _lastFrameAnimDelta = _lastFrameAnimDelta! + delta * (1 - _baseAnim.value);
  ...
  void _updateDelta(double delta) {
    _totalDragDelta = _totalDragDelta! + delta;
  ...
  void _updateOverDragOffset() {
    ...
    _overDragDelta = _overDragDelta! + offsetDelta;
  }
